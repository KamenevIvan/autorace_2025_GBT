...
class Competition(Node):
    def __init__(self):
        ...
        # Для конвертации изображений
        self.bridge = CvBridge()
        
        self.get_logger().info('Competition node started!')
        self.get_logger().info(f'Tolerance: {self.tolerance_percent}')
        self.get_logger().info(f'Track color: {self.track_color}')
    ...
    def detect_turn_direction(self, image):
        ...
        # 2. Обрезаем изображение по квадрату вокруг круга
        x1 = max(0, x - r)
        y1 = max(0, y - r)
        x2 = min(w, x + r)
        y2 = min(h, y + r)
        ...
        left_part = binary_with_stripe[:, :r]  # Первые r пикселей слева
        right_part = binary_with_stripe[:, -r:]  # Последние r пикселей справа
        ...
        try:
            ...
            if self.stage == 3:
                ...
                if self.direction == -1:
                    ...
                    self.get_logger().info(f'DIRECTION = LEFT')
                else:
                    self.get_logger().info(f'DIRECTION = LEFT')
                    self.direction == 1
